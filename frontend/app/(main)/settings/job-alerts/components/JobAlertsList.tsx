...
export function JobAlertsList({
  ...
  const activeAlertsCount = alerts.filter(
    (alert) => alert.isActive && !alert.isPaused,
  ).length;
    ...
    <div className="space-y-6">
      ...
      {activeAlertsCount >= 8 && activeAlertsCount < 10 && (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            You are approaching the limit of 10 active job alerts. You have{" "}
            {10 - activeAlertsCount} slots remaining.
          </AlertDescription>
        </Alert>
      )}
      ...
      {isAtLimit && (
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            You have reached the maximum of 10 active job alerts. Delete or
            pause an alert to create a new one.
          </AlertDescription>
        </Alert>
      )}
      ...
      {alerts.length === 0 ? (
        <div className="py-12 text-center">
          <div className="mx-auto max-w-md">
            <h3 className="mb-2 text-lg font-semibold">No job alerts yet</h3>
            <p className="text-muted-foreground mb-4">
              Create your first job alert to receive notifications when matching
              jobs are posted.
            </p>
