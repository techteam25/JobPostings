...
SERVICE_NAME="jobposting-backend"
DEPLOY_DIR="/var/www/job-posting-backend"
BACKUP_DIR="/var/www/job-posting-backend.backup.$(date +%s)"
TARBALL="/tmp/backend-deploy.tar.gz"
...
sudo -u jobpostinguser NODE_ENV=production bun run build
...
ls -dt /var/www/job-posting-backend.backup.* 2>/dev/null | tail -n +4 | xargs rm -rf || echo "No old backups to clean"
