...
export const jobs = mysqlTable('jobs', {
  ...
  jobType: mysqlEnum('job_type', ['full-time', 'part-time', 'contract', 'freelance', 'internship']).notNull(),
  experienceLevel: mysqlEnum('experience_level', ['entry', 'mid', 'senior', 'lead', 'executive']).notNull(),
  ...
  employerIdx: index('employer_idx').on(table.employerId),
...
export const jobApplications = mysqlTable('job_applications', {
  ...
  status: mysqlEnum('status', ['pending', 'reviewed', 'shortlisted', 'rejected', 'hired']).default('pending').notNull(),
  ...
  applicantIdx: index('applicant_idx').on(table.applicantId),
...
export const insertJobSchema = createInsertSchema(jobs, {
  ...
  salaryCurrency: z.string().length(3, 'Currency must be 3 characters').optional(),
  requiredSkills: z.string().optional(), // Will be JSON string
  employerId: z.number().positive('Employer ID is required'),
});

export const selectJobSchema = createSelectSchema(jobs);
export const updateJobSchema = insertJobSchema.partial().omit({ id: true, createdAt: true });
...
export const publicJobSchema = selectJobSchema.omit({
  employerId: true,
}).extend({
  employer: z.object({
    id: z.number(),
