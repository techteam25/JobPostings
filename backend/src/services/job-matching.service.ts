...
import { Job, JobApplication } from '../db/schema/jobsDetails';
...
export class JobMatchingService extends BaseService {
  ...
  async getRecommendedJobs(
    ...
    try {
      ...
      const userApplications = await this.jobRepository.findApplicationsByUser(userId);
      const appliedJobIds = userApplications.map(app => app.job?.id).filter(Boolean);

      // Get all active jobs
      const activeJobs = await this.jobRepository.findActiveJobs({ page: 1, limit: 1000 });
  ...
  async findCandidatesForJob(
    jobId: number,
    options: { page?: number; limit?: number } = {}
  ): Promise<PaginationResult<{ user: SafeUser; score: number; matchReasons: string[] }>> {
    try {
      const { page = 1, limit = 10 } = options;
      ...
      const activeUsers = await this.userRepository.findActiveUsersByRole('user');

      // Get users who haven't applied for this job
      const jobApplications = await this.jobRepository.findApplicationsByJob(jobId);
      const appliedUserIds = jobApplications.map(app => app.applicant?.id).filter(Boolean);
  ...
  private async calculateJobMatchScore(
    ...
    } else if (user.profile?.city && job.location.toLowerCase().includes(user.profile.city.toLowerCase())) {
      score += 20;
      matchReasons.push('Location match');
    } else if (user.profile?.state && job.location.toLowerCase().includes(user.profile.state.toLowerCase())) {
      score += 10;
    ...
    if (job.requiredSkills && user.profile?.bio) {
      ...
      if (matchedSkills.length > 0) {
        ...
        score += skillScore;
        matchReasons.push(`Matching skills: ${matchedSkills.slice(0, 3).join(', ')}`);
      }
    }
    ...
    } else if (this.isCompatibleExperienceLevel(userExperienceLevel, job.experienceLevel)) {
      score += 15;
    ...
    } else if (job.jobType === 'remote' || job.isRemote) {
      score += 15;
  ...
  private async calculateUserJobMatchScore(
    ...
    return this.calculateJobMatchScore(userWithProfile, job);
  }
  ...
  private calculateJobSimilarity(job1: Job, job2: Job): number {
    ...
    if (job1.requiredSkills && job2.requiredSkills) {
      ...
      if (skills1.length > 0 && skills2.length > 0) {
        const skillSimilarity = (matchedSkills.length / Math.max(skills1.length, skills2.length)) * 20;
  ...
  private extractSkills(text: string): string[] {
    ...
    const commonSkills = [
      ...
      'docker', 'kubernetes', 'aws', 'azure', 'git', 'linux', 'mongodb', 'postgresql',
      'leadership', 'communication', 'project management', 'teamwork', 'problem solving',
      'marketing', 'sales', 'customer service', 'accounting', 'finance', 'hr',
    ];
  ...
  private estimateUserExperienceLevel(user: SafeUser): string {
    // This would analyze work experience duration
    // For now, return a default value
    return 'mid';
  }

  private isCompatibleExperienceLevel(userLevel: string, jobLevel: string): boolean {
  ...
  private isSimilarLocation(location1: string, location2: string): boolean {
    ...
    const clean1 = location1.toLowerCase().replace(/[,\s]+/g, ' ').trim();
    const clean2 = location2.toLowerCase().replace(/[,\s]+/g, ' ').trim();
